...
def initial():
    ...
    if goal_cords_x<1 or goal_cords_x>400 or goal_cords_y<1 or goal_cords_y>250:
        print("wrong input")
        exit(0)
    else:
...
for i in range(len(All_list)):
    x = All_list[i].node_loc[1]
    y1 = All_list[i].node_loc[0]
    canvas_for_backtrack[(249-y1,x)]=[255,0,0] #setting every backtracked pixel to white
new_backtracked = cv2.resize(canvas_for_backtrack,(1200,750))
...
for i in range(len(x_a)):
    ...
    y2 = x_a[i]
    # y = abs(249-y2)
    canvas_for_backtrack[(249-y2,x)]=[0,255,0] #setting every backtracked pixel to green
#showing the final backtracked path
# canvas_for_backtrack = np.flipud(canvas_for_backtrack)
new_backtracked = cv2.resize(canvas_for_backtrack,(1200,750))
